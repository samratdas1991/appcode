# Screen Specification: _a-0000e2cd-bc96-8000-9ba2-011c48011c48_144143.xml

## Screen Overview
- **Purpose:** User interface component
- **Source File:** `content/_a-0000e2cd-bc96-8000-9ba2-011c48011c48_144143.xml`
- **Component Count:** 3

## Components

| Component | Appian SAIL | Pega UI Element |
|-----------|------------|-----------------|
| a!columnsLayout | SAIL columnsLayout | Two Column / Dynamic Layout |
| a!formLayout | SAIL formLayout | Section with layout (pxLayout) |
| a!richTextDisplayField | SAIL richTextDisplayField | pxDisplayText |

## Rule Inputs (Data Bindings)

| Parameter | Pega Mapping |
|-----------|-------------|
| ri!reason | Case property or Page property |
| ri!request | Case property or Page property |

## Local Variables

| Variable | Pega Equivalent |
|----------|----------------|
| local!actualCurrencyCode | Local action variable or clipboard page |
| local!i18nData | Local action variable or clipboard page |
| local!openTaskCount | Local action variable or clipboard page |
| local!originalFundingDate | Local action variable or clipboard page |
| local!reasonChoices | Local action variable or clipboard page |
| local!reasonShowWhen | Local action variable or clipboard page |

## Pega Implementation Notes

- Recreate as Pega Section rule
- Map rule inputs to section parameters or case properties
- Convert local variables to clipboard pages or local actions
- Apply Pega Cosmos design system for consistent UI